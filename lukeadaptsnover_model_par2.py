...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 11 10:55:41 2023
...
with h5py.File(train_dataname, "r") as f:
    ...
    #X_train = np.reshape(X_train, (n,o,p,1))
    #X_val = np.reshape(X_val, (n,o,p,1))
    #X_val = X_train
...
with strategy.scope():
    ...
    shape_before_flattening  = model.shape #upgraded syntax from Keras.backend.int_shape
    X = Flatten()(model)
    ##############################################ENCODING###############################################
    ...
    early_stop = EarlyStopping(monitor='val_loss', min_delta=0, patience=1, verbose=1,
                               mode='min', restore_best_weights=True)

    optim =  tf.keras.optimizers.SGD(learning_rate=0.001)           #Synchronised learning rate optimization algorithm (Adam)
    loss = 'mse'  
    lossfun = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)                             #Mean Squared Error Loss function
    encoder.compile(loss=lossfun,optimizer=optim) 
    autoencoder.compile(loss=lossfun,
                    optimizer=optim,
